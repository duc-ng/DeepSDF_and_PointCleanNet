    ...
    def forward(self, x):
        ...
        if self.num_scales == 1:
            ...
            if x.is_cuda:
                ...
                x_scales = Variable(torch.FloatTensor(x.size(0), 1024*self.num_scales, 1)).to(x.device)
        ...
        if not self.quaternion:
            ...
            if x.is_cuda:
                ...
                trans = Variable(torch.FloatTensor(batchsize, 3, 3)).to(x.device)
    ...
    def forward(self, x):
        ...
        if self.num_scales == 1:
            ...
            if x.is_cuda:
                ...
                x_scales = Variable(torch.FloatTensor(x.size(0), 1024*self.num_scales**2, 1)).to(x.device)
            if self.sym_op == 'max':
                for s in range(self.num_scales):
