...
def parse_arguments():
    ...
    parser.add_argument('--workers', type=int, default=8, help='number of data loading workers - 0 means same thread as main execution')
...
def eval_pcpnet(opt):
    ...
    regressor = ResPCPNet(
        ...
        point_tuple=trainopt.point_tuple)
    regressor.load_state_dict(torch.load(model_filename, map_location=torch.device(DEVICE)))
    regressor.to(DEVICE)
    shape_ind = 0
    ...
    model_outdir = os.path.join(opt.outdir)
        ...
        with torch.no_grad():
            ...
            for oi, o in enumerate(trainopt.outputs):
                if o == 'unoriented_normals' or o == 'oriented_normals':
                    ...
                    o_pred_len = torch.max(torch.tensor([sys.float_info.epsilon*100], device=DEVICE), o_pred.norm(p=2, dim=1, keepdim=True).to(DEVICE))
                    ...
                    o_pred = torch.mul(o_pred, torch.t(patch_radiuses.expand(3, n_points)).float().to(DEVICE)) + originals.to(DEVICE)
                    pred[:, output_pred_ind[oi]:output_pred_ind[oi]+3] = o_pred
                elif o in ['outliers']:
                    ...
                    o_pred = originals.to(DEVICE)

                    o_pred = torch.cat((o_pred, outliers_value.to(DEVICE)), 1)
                    pred = o_pred
            ...
            while batch_offset < pred.size(0):
                ...
                if shape_patches_remaining <= batch_patches_remaining:
                    ...
                    if len(oi) > 1:
                        ...
                        np.savetxt(os.path.join(model_outdir, dataset.shape_names[shape_ind]+'.info'), outliers_prop.numpy())
                        prop_saved[oi] = True
                    # save curvatures
